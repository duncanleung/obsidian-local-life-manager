---
name: ops-git-sync
description: "Sync all git repos - my-life top repo and code repos (from Projects Index) projects"
...
Synchronize all git repositories: the top-level `my-life` repo and the `code repos (from Projects Index)` projects.
...
### Index → Filesystem
- Each project with `code:` path exists in code repos (from Projects Index)
- Git remote matches `remote:` in index
- Branch status (ahead/behind/dirty/diverged)

### Filesystem → Index
- Finds repos in code repos (from Projects Index) not listed in CLAUDE.md
...
## Not in Index
  ...
  ? django-tutorial
    Path: code repos (from Projects Index)django-tutorial
...
# Add to CLAUDE.md Projects Index:
  ...
  - name: django-tutorial
    code: code repos (from Projects Index)django-tutorial/
    remote: https://github.com/user/django-tutorial.git
    branch: main
