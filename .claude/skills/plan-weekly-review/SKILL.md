...
1. Check if this week's note exists: `02 Calendar/YYYY-Www.md`
2. If not, create from `08 System/Templates/Weekly Template.md`
...
- "# ![[slack-logo.png|18]] Slack Conversations" - should have content if it was a workday (Mon-Fri)
...
```bash
gh search commits --author=duncanleung --committer-date=YYYY-MM-DD..YYYY-MM-DD --limit=50
```
...
### Project Progress
1. Scan `"06 Projects"/*/issues/` for any WORKLOG.md updates this week
2. Note completed tasks, status changes

## Step 4: Fill In Weekly Note

Update the weekly note (`02 Calendar/YYYY-Www.md`) sections:
- **What Went Well**: Ask the user
- **What Didn't Go Well**: Ask the user
...
1. "What's the one thing that would make next week a success?"
2. "Any work priorities or deadlines this week?"
...
Update `.claude/obsidian-memories/about-me.md` if job status or major context changed.
