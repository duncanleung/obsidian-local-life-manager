...
2. **For each enabled feed:**
   ...
   - **Deduplicate against existing notes:**
     ```bash
     grep -rl "media: {article_url}" "01 Inbox/Articles/"
     ```
...
4. **Create discovery notes:**
   ...
   - Search vault to check if a note already exists for each discovery
   - For new discoveries, create a note in `01 Inbox/`
...
**IMPORTANT: Read `08 System/Tag Index.md` before processing to verify valid tags.**
...
Tags should be from the canonical list in `08 System/Tag Index.md`.
...
- Skip articles that already exist in the vault
