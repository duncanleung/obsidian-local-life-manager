...
3. **Deduplicate against existing notes:**
   - Before processing, check if a note with that video URL already exists:
     ```bash
     grep -rl "youtube.com/watch?v={video_id}" "01 Inbox/Videos/"
     ```
...
STEPS:
1. Run: cd .claude/skills/research-youtube-catchup && python3 scripts/youtube_helper.py full {video_id}
...
See `references/note-template.md` for the full video note template, summary guidelines, tutorial format, duration formatting, title sanitization, discovery note format, and path handling rules. Both this skill and `/research-video-summarize` use the same template.
