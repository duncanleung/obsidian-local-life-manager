---
name: session-improve-processes
...
```bash
/session-improve-processes                    # Reflect on entire session
/session-improve-processes --focus agents     # Focus on agent improvements
/session-improve-processes --focus commands   # Focus on command improvements
/session-improve-processes --focus templates  # Focus on template improvements
...
**Create issue:**
- Create in `06 Projects/meta-repo/issues/`
...
```bash
/session-improve-processes --focus templates
```
...
```
[work session] → /session-improve-processes → [improvements] → [next session]
```
